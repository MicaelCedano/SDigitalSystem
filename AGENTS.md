...
## 📋 Tareas Recientes (2026-06-27)
- [x] **Eliminar paso de QC en desbloqueos (2026-06-27)**: el técnico crea solicitud en estado "Pendiente Admin" directo (sin pasar por QC). Micael aprueba y se paga solo al técnico (RD$25/IMEI), ya no se paga al QC. Funciones eliminadas: `qcRevisarSolicitud`, `recordarQCsDesbloqueos`. Página `/qc/desbloqueos` y componente `QCDesbloqueosClient.tsx` borrados. Banner ámbar "Recordar al QC" removido del admin. Validación anti-doble-pago contra `unlockRecord` se mantiene en `crearSolicitudDesbloqueo` (mismo rigor, mismo momento). Script SQL `prisma/migrations/20260627_migrate_pendiente_qc_a_admin.sql` para mover solicitudes viejas atrapadas. Resumen de pago en admin ahora muestra solo "al técnico". Si quedaron solicitudes "Pendiente QC" antes del cambio, Micael las corre en Supabase.
- [x] **Desglose explícito del pago en notificación Telegram (2026-06-27)**: el mensaje de QC→admin ahora dice "RD$25 (técnico) + RD$25 (QC) = RD$50 total" en vez del confuso "Pago total RD$50" que hacía pensar que el técnico cobraba 50. Commit `fe3870e`.

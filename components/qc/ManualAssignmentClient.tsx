...
interface Props { qcUsers: QCUser[]; }
...
export default function ManualAssignmentClient({ qcUsers }: Props) {
        ...
        <div className="space-y-5">
            ...
            <div className="bg-[#0f172a] rounded-[2rem] p-5 flex flex-col md:flex-row items-start md:items-center gap-5 relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 flex items-center gap-5">
                    ...
                    <div className="text-center">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Est.</p>
                        <p className="text-2xl font-black text-indigo-300 leading-none font-mono">RD${(okCount * 50).toLocaleString()}</p>
                    </div>
                </div>
                ...
                <div className="relative z-10 flex-shrink-0">
                    ...
                    {(errorCount + dupCount) > 0 && okCount > 0 && (
                        <p className="text-[10px] text-amber-400 font-bold text-center mt-1.5 flex items-center justify-center gap-1">
                            <AlertCircle size={10} />{errorCount + dupCount} inválidos serán ignorados
                        </p>
                    )}
                </div>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                {/* IMEI Textarea */}
                <div className="bg-white rounded-[2rem] shadow-lg shadow-slate-200/50 border border-slate-100 p-6 flex flex-col gap-4">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-indigo-50 rounded-xl text-indigo-600"><Smartphone size={18} /></div>
                        <div>
                            <h3 className="font-bold text-slate-800">Lista de IMEIs</h3>
                            <p className="text-xs text-slate-400">Un IMEI por línea, o pega un bloque desde Excel</p>
                        </div>
                        {checkingCount > 0 && (
                            <div className="ml-auto flex items-center gap-1.5 text-xs font-bold text-indigo-500">
                    ...
                    <Textarea
                        ...
                        className="flex-1 min-h-[380px] p-4 bg-slate-50 border-slate-200 rounded-2xl focus:bg-white font-mono text-sm resize-none leading-7"
                        autoFocus
                    />
                    {imeiStatuses.length > 0 && (
                        <div className="grid grid-cols-4 gap-2">
                            {[
                                { label: "Válidos", val: okCount, cls: "bg-emerald-50 border-emerald-100 text-emerald-700" },
                                { label: "Errores", val: errorCount, cls: "bg-rose-50 border-rose-100 text-rose-700" },
                                { label: "Duplic.", val: dupCount, cls: "bg-orange-50 border-orange-100 text-orange-700" },
                                { label: "Revisan.", val: checkingCount, cls: "bg-amber-50 border-amber-100 text-amber-700" },
                            ].map(({ label, val, cls }) => (
                                <div key={label} className={`p-2.5 border rounded-2xl text-center ${cls}`}>
                                    <p className="text-[9px] font-black uppercase tracking-widest opacity-70">{label}</p>
                                    <p className="text-xl font-black leading-none mt-0.5">{val}</p>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="bg-white rounded-[2rem] shadow-lg shadow-slate-200/50 border border-slate-100 p-6 flex flex-col gap-4">
                    <div className="flex items-center gap-3">
                        <h3 className="font-bold text-slate-800">Estado en Tiempo Real</h3>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 max-h-[460px] pr-1">
                        {imeiStatuses.length === 0 ? (
                            <div className="flex flex-col items-center justify-center text-center py-20 text-slate-200">
                                <Smartphone size={52} className="mb-4" />
                                <p className="font-medium text-sm text-slate-400">Los resultados aparecerán<br />conforme ingreses IMEIs</p>
                            </div>
